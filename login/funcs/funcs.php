<?php
	...
	function usuarioExiste($usuario)
	{
		...
		$stmt = $mysqli->prepare("SELECT id_usuario FROM usuario WHERE usuario = ? LIMIT 1");
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->Username = 'socialhealthlog@gmail.com'; //Modificar
		$mail->Password = 'administrador'; //Modificar
		
		$mail->setFrom('SocialHealthlog@gmail.com', 'SocialHealth'); //Modificar
	...
	function isFirstTime($id){
		global $mysqli;

		$stmt = $mysqli->prepare("SELECT id_usuario , last_session password FROM usuario WHERE id_usuario = ? LIMIT 1");
		$stmt->bind_param("s", $id);
		$stmt->execute();
		$stmt->store_result();
		$rows = $stmt->num_rows;

		$stmt->bind_result($id,$last_session);
		$stmt->fetch();
		//echo "$last_session";
		if($last_session == "0000-00-00 00:00:00"){
			return true;
		}else
		{
			return false;
	...
	function login($usuario, $password)
	{
		...
		$stmt = $mysqli->prepare("SELECT id_usuario, id_tipo, password FROM usuario WHERE usuario = ? || correo = ? LIMIT 1");
